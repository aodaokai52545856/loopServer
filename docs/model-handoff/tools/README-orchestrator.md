...
| ② | **生成提示词并提交 generated** | 调用 PS1 生成执行提示词并 commit；提示词含「编排器附注」说明 READY 时 HEAD 可为该 docs 提交；工作区保持干净 |
| — | **OpenCode 粘贴执行** | 人工：将弹窗/剪贴板内容粘贴到 OpenCode，完成实现 |
| ③ | **标记执行完成** | 记录 Task result commit（可选覆盖 SHA） |
| ④ | **生成复核提示词** | 生成并复制复核提示词 |
