...
| ② | **生成提示词（先不 commit）** | 调用 PS1 生成执行提示词并复制；**先不入库**，在提示词里声明 `generated/*.md` 为已知未跟踪偏差，避免 Base/HEAD 因提前 commit 错位 |
| — | **OpenCode 粘贴执行** | 人工：将剪贴板/弹窗内容粘贴到 OpenCode，完成实现 |
| ③ | **标记执行完成** | 记录 Task commit；若仍有未提交的 generated 提示词，此时再 `docs(handoff): add prompt` |
| ④ | **生成复核提示词** | 生成并复制复核提示词 |
