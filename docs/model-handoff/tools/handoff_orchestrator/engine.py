...
class Orchestrator:
    ...
    def generate_and_commit_prompt(self) -> str:
        """Generate launch prompt, commit it for a clean tree, and reconcile Base/HEAD rules.

        Base stays the pre-prompt tip. HEAD becomes the docs(handoff) prompt commit.
        A trusted 编排器附注 tells the model READY may accept that HEAD.
        """
        ...
        prompt = append_base_head_reconcile_note(prompt, self.ctx.current_task_id)
        ...
        self.log(
            "INFO",
            f"prompt committed: {rel} (HEAD={head[:12]}, clean tree). "
            "READY must follow 编排器附注 for Base/HEAD. Opening copy dialog.",
        )
